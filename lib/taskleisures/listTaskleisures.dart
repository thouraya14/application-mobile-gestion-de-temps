...
import 'package:devapp/personaltask/add_task_bar.dart';
...
class ListLeisuresPage extends StatefulWidget {
  const ListLeisuresPage({Key? key}) : super(key: key);

  @override
  _ListLeisuresPageState createState() => _ListLeisuresPageState();
}

class _ListLeisuresPageState extends State<ListLeisuresPage> {
  ...
  void fetchTasks() async {
    try {
      final QuerySnapshot<Map<String, dynamic>> querySnapshot =
          await FirebaseFirestore.instance.collection('taskLeisures').get();
      setState(() {
  ...
  return MaterialApp(
    ...
    home: Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('List Tasks Leisures'),
        actions: [
          IconButton(
            onPressed: () {
              setState(() {
        ...
        onTap: (index) {
          setState(() {
            ...
            switch (_selectedIndex) {
              case 0:
                Navigator.pushReplacement(context, MaterialPageRoute(builder: (context) =>const ListLeisuresPage()));
                break;
              case 1:
